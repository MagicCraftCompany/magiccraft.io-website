...
James requested the visible lobby-to-NFT-to-game story on 6 September 2026. `Web3JourneySection.tsx` presents official character artwork and a separately labelled game character preview, Genesis rarity tiers, and links to the existing lobby and marketplace. Preserve this product direction and its optional-wallet boundary; `AiHomepage.test.tsx` checks the mounted journey and handoffs. James also requested an authentic glowing NFT image. The game preview now uses a still at 00:02 from [MagicCraft’s official Doge NFT skin reel](https://www.instagram.com/reel/DIlsE8Hqt7B/) (18 April 2025), stored locally at `public/gameplay/doge-nft-glow.svg` as a self-contained JPEG frame inside an SVG viewport that excludes the promotional banners. This text asset can be maintained through the existing GitHub web editor without adding upload permissions. It shows the gold Doge skin glowing in the actual game hero screen, with the source label “LIMITED NFT SKIN.” Preserve the original appearance and the source link; do not add an artificial glow or use expiring social CDN URLs. The visible caption identifies Doge separately from the Karas Genesis artwork. This source does not establish Doge’s Genesis/Revelation membership or Legendary rarity. Exact collection/client glow eligibility still needs a current source; do not imply that every Legendary NFT receives this skin. Official sources: [Genesis](https://docs.magiccraft.io/nft-collections/genesis-collection), [NFT utility](https://docs.magiccraft.io/nft-collections/nft-utilities), [Web3 lobby](https://docs.magiccraft.io/magiccraft-web3-integration-system/web3-lobby), and the public marketplace catalog. These document the product story, not a wallet transaction or game-client acceptance test.
