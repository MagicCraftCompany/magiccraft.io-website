...
Last audited: 13 July 2026, approximately 19:37 ICT
...
### Local work-in-progress snapshot

Treat every item here as `LOCAL-PENDING`, not deployed:
...
- The broken Rent link has been removed from the local drawer data.
- Game Maker now points to the real Steam listing and explains that game integration is still planned.
- Local shared-shell/back-navigation changes exist for Careers, Bounties, Privacy, and Terms.
- Focused navigation and page-shell tests exist locally, but the entire working tree still requires integrated verification and production deployment proof.
...
- [x] `CODE-COMPLETE, LIVE-PENDING` Finish the fact-checked whitepaper rewrite; remove investment-like, fixed-return, universal-MCRT, shared-account, and unsupported product claims.
- [x] `CODE-COMPLETE, LIVE-PENDING` Verify the shared AI catalog, drawer purpose/status labels, one-open accordion, scroll lock, focus trap, Escape behavior, active-route state, and external-link indicators.
- [ ] `LOCAL-PENDING` Confirm Rent is absent after deployment and Game Maker opens the exact Steam listing.
- [x] `CODE-COMPLETE, LIVE-PENDING` Replace `/server` fabricated statuses and logs with an honest service directory and source-backed Stats path.
- [x] `CODE-COMPLETE, LIVE-PENDING` Hand `/leaderboard` to the working lobby rankings and remove the permanent loader and wrong “Verify” heading.
- [x] `CODE-COMPLETE, LIVE-PENDING` Remove placeholder holder rows and inert pagination; hand current balances to the attributable BNB Chain explorer.
- [x] `CODE-COMPLETE, LIVE-PENDING` Correct `/verify`: remove Rent while broken and delete unsupported phone/WeChat claims.
- [x] `CODE-COMPLETE, LIVE-PENDING` Restore useful lobby totals through the owned `/stats` path without loading the broken empty charts.
- [ ] `BLOCKED-UPSTREAM` Restore season/battle-pass data after a reachable `GAMESERVER_API_URL`, valid API key, and server/firewall access are provided; all legacy regional hosts currently time out.
- [x] `CODE-COMPLETE, LIVE-PENDING` Make grant submission fail closed: redirect only after the intake service accepts it, validate server-side, and keep direct success-page visits neutral. A real submission remains gated.
...
- [x] `CODE-COMPLETE, LIVE-PENDING` Make `/grants/success` neutral when visited directly and show acceptance only after the accepted intake redirect.
- [ ] Run the full integration gate, deploy the exact tested commit, and re-run production route, function, link, and rendered mobile checks.
...
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------- |
| `/`                    | Ecosystem homepage; explain the AI portfolio and MagicCraft game, then route users to the correct product or game destination. | Shared AI catalog/home cards are `LOCAL-PENDING`; render Home, every card, hero CTAs, mobile bottom bar, and fallback states.                  | `[critical]`                |
...
| `/stats`               | Show validated lobby totals and current MCRT market data without depending on empty chart series.                              | `CODE-COMPLETE`: public lobby totals are bounded and mapped server-side; season fields stay unavailable while game-server access is blocked.   | `[critical]`                |
...
| ------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| Explore AI Suite / AI Suite Overview -> `/#ai-products` | Scroll to the single comparison of verified AI products.                                                                          | `LOCAL-PENDING` router/hash handling.                             | Open from another route and Home; drawer must close and land on the section.                           |
...
| Drawer item                                                     | Intended function                                                   | Audit status                                                                    | Next acceptance                                                                             |
| --------------------------------------------------------------- | ------------------------------------------------------------------- | ------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| Web3 Lobbies -> `https://lobby.magiccraft.io/`                  | Create or join supported matches and manage Web3 game rewards.      | `LIVE-HTTP` 200.                                                                | Anonymous landing only; account, wallet, match, and reward proof are `GATED`.               |
| Marketplace -> `https://app.magiccraft.io/marketplace/explorer` | Browse and trade supported game assets.                             | `LIVE-HTTP` 200.                                                                | Browse/filter read-only; wallet connection, listing, and trade are `GATED`.                 |
| Pledging -> `https://app.magiccraft.io/pledging`                | Lock MCRT for a selected term under the current pool rules.         | `LIVE-HTTP` 200.                                                                | Verify current terms/rate presentation read-only; approval and pledge are `GATED`.          |
| Referral System -> `https://lobby.magiccraft.io/referral`       | Create or use a referral path for eligible lobby rewards.           | `LIVE-HTTP` 200.                                                                | Verify eligibility copy; sign-in, code creation, and reward proof are `GATED`.              |
| Rent (Testnet) -> `https://rent.magiccraft.io/`                 | Former testnet rental destination shown in the supplied screenshot. | `KNOWN-BROKEN`: HTTP 403, Cloudflare Error 1000. Locally removed, not deployed. | `[gated]` DNS/hosting repair, then product-owner and full testnet review before re-listing. |
...
| Whitepaper -> `/whitepaper`                   | Verified living guide and source ledger.                   | Critical rewrite pending.                                 | Fact regression test, mobile TOC, print, and production review. |
| Game Overview -> `/magiccraft`                | Current game, modes, story, and platforms.                 | Local rename from “MagicCraft Vision” pending deployment. | Current-content and device CTA proof.                           |
...
| Game Maker -> Steam app `3478810`                        | Download the free MCRT Game Maker map editor. Export/integration remains planned. | `LIVE-HTTP` 200; corrected local destination is pending deployment.   | Store truth/read-only check; install/export is `GATED`.                                        |
...
| Game stats -> `/stats`                                   | Show validated lobby totals and current market data.                              | `CODE-COMPLETE`: owned crash-safe fallback uses the public Stats API. | Keep the older external chart regression documented until its source app filters empty series. |
...
| ------------------------------------------------------ | -------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------- |
| `battlepass.ts` via `/gameserverapi/battlepass/active` | Authenticated server-side proxy for current battle-pass data.                          | `KNOWN-BROKEN`: HTTP 502.                                                                        | `[critical]` repair upstream URL/key/network and add timeout/error contract. |
| `game-stats.ts` via `/api/game-stats`                  | Combine lobby, game-server, and market data without inventing missing values.          | `CODE-COMPLETE`: bounded lobby mapping with source metadata; legacy game server still times out. | `[critical]` deploy/verify fallback; recover season upstream separately.     |
