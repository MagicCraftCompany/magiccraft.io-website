...
| Game Stats                              | `LIVE-PARTIAL`  | Production renders verified lobby and market data: 322,766 lobbies, 28,166 users, 100,642 finished lobbies, and 145.83M MCRT entry fees. All three regional GameServers answer on `8903` from independent non-Netlify networks, while Netlify alone times out, so season fields remain explicitly unavailable. Publish each owned route over HTTPS 443, then configure the matching regional endpoint. |
...
- [ ] `BLOCKED-UPSTREAM` Restore season/battle-pass data. All three GameServers answer on `8903` from the Mac and an independent server, while Netlify alone times out. Publish the owned routes over HTTPS 443, configure `GAMESERVER_API_URL_EUROPE`, `GAMESERVER_API_URL_ASIA`, and `GAMESERVER_API_URL_AMERICA`, then rotate the historical credential and store only the replacement in Netlify environment storage.
...
- [ ] Fix `rent.magiccraft.io` Cloudflare Error 1000 only with authorized Render and DNS access. The public DNS-only CNAME reaches the former Render hostname, but Render returns 404 there and has no working custom-host mapping. Restore or correct the Render service, add and verify `rent.magiccraft.io` as its custom domain, and keep Rent hidden until a post-fix production check passes.
...
| ------------------------------------------------------ | -------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `battlepass.ts` via `/gameserverapi/battlepass/active` | Authenticated server-side proxy for current battle-pass data.                          | `KNOWN-BROKEN`: HTTP 502. Regional servers answer outside Netlify, but Netlify reports `upstream_timeout`; code now requires regional or shared HTTPS endpoints instead of cleartext `8903`. | `[critical]` publish the owned regional routes over HTTPS 443 and configure their Netlify environment names. |
