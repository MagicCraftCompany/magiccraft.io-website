...
export const useBattlePass = (selectedRegion: Region = 'europe') => {
  const [battlePassData, setBattlePassData] = useState<BattlePassData | null>(
    null
  )
  ...
  const fetchBattlePassData = useCallback(async () => {
    try {
      ...
      if (
        import.meta.env.DEV &&
        import.meta.env.VITE_USE_MOCK_BATTLEPASS === 'true'
      ) {
        setBattlePassData({
          id: 'battlepass_season_1',
          name: 'MAGICCRAFT BATTLEPASS',
          description:
            'The 90-Day Challenge Has Begun! Stay at the top of the leaderboard to claim rewards and collect MCRT!',
          startDate: new Date(
          ...
          topParticipants: [
            ...
            {
              playerId: 'player_2',
              playerName: 'CryptoKnight',
              currentScore: 8500,
              currentRank: 2,
            },
            {
              playerId: 'player_3',
              playerName: 'MagicMaster',
              currentScore: 7200,
              currentRank: 3,
            },
            {
