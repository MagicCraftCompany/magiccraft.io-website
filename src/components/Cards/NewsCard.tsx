...
export function NewsCard({ article, className = '' }: NewsCardProps) {
  return (
    <a
      ...
      className={`group relative block cursor-pointer overflow-hidden rounded-3xl bg-[#070725] shadow-lg transition-transform hover:scale-[1.02] ${className}`}
    >
      <div className="relative h-full w-full p-6">
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            {article.dateLabel && (
              <time className="text-xs text-white/45">{article.dateLabel}</time>
            )}
            {article.version && (
          ...
          <h3 className="text-2xl font-bold text-white transition-colors group-hover:text-teal-400">
            {article.title}
          </h3>
        ...
        <div className="absolute right-0 top-0 h-full w-1/3">
          <div className="relative h-full w-full">
            <img src={article.image} alt="" className="object-cover" />
